...
export class ShowService {
   ...
   */
  showPositions() {
    this.httpClient.get<any[]>(environment.endpoints.hereiam).subscribe({
      next: (response) => {
        ...
        this.response.message =
          nbTraces +
          " traces enregistrées - dernière en date le " +
          this.datepipe.transform(lastDate) +
          " a proximité de  " +
          lastCodePostal +
          " " +
          lastCommune;
      ...
      error: (error) => {
        this.response.datas = [];
        this.response.message =
          "Echec du chargement des données  " + error.message;
